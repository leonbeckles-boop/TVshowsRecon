...
async def _try_show_query(db: AsyncSession, sid: int, sql: str) -> Optional[Dict[str, Any]]:
    """Try a single show summary query.

    IMPORTANT: if a statement fails inside a transaction (e.g. missing column),
    Postgres marks the transaction as aborted until we ROLLBACK. Without this,
    subsequent fallback queries will *all* fail with:
      'current transaction is aborted, commands ignored until end of transaction block'
    """
    try:
        row = await db.execute(text(sql), {"sid": sid})
        r = row.mappings().first()
        return dict(r) if r else None
    except Exception:
        # Clear the failed transaction so the next fallback query can run.
        try:
            await db.rollback()
        except Exception:
            # If rollback itself fails, we still want to swallow to keep behavior consistent.
            pass
        return None
