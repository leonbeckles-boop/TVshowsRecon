...
async def _fetch_show_from_db(db: AsyncSession, tmdb_id: int) -> Optional[Dict[str, Any]]:
    """
    Schema-safe lookup in `shows`.

    Render DB schema uses:
      - show_id (TMDb id)
      - title
      - year
      - poster_path
    ...
    We use ONE robust query and COALESCE to avoid trying non-existent columns
    (e.g. poster, release_year) which can poison the transaction.
    """
    q = text(
        """
        SELECT
    ...
    try:
        ...
        poster_url = _poster_url_from_path(poster_url) if poster_url else None
